...
- **Message durability**: Every message Rondel has accepted responsibility for is persisted before it's acknowledged. Inbound channel messages, subagent results, inter-agent mail — all flow through the Router's disk-backed queue at `state/queues/{encodedConversationKey}.json` (one file per conversation key, atomic writes, corruption quarantine, delete-on-empty — same pattern as `state/inboxes/`). Memory is a cache of disk. On startup, `Router.recoverQueues()` rebuilds the in-memory view and drains naturally as agents reach idle. Delivery is at-least-once: a crash between dispatch and disk-remove replays the message on recovery. Loss would be strictly worse than duplication for this use case.
...
- **Session resilience**: New session entries only persist to disk after Claude CLI confirms the session. Resume failure detection falls back to fresh session within 10s regardless of exit code. Two layers: prevention (don't write stale entries) + recovery (detect and recover from stale entries). `persistSessionIndex` is serialized via `AsyncLock` so concurrent rotations (e.g. `sessionEstablished` firing while `resetSession` is mid-flight) write in enqueue order; fire-and-forget persists log their errors via `persistInBackground` instead of silently swallowing them.
...
- **One writer per conversation at a time**: A conversation is a serial execution context. Never send a second message to an agent process that's already busy — queue it and drain on idle. This is a correctness invariant (not just a Claude CLI limitation), enforced structurally by the Router's per-conversation `AsyncLock` around every send/enqueue decision. `process.sendMessage` serializes stdin but not the decision of *what to send next* — the lock closes that TOCTOU window. The `sendOrQueue` pattern is the canonical approach for any internal message injection (subagent results, cron delivery, inter-agent messages). For agent-mail conversations, `sendOrQueue` carries `AgentMailReplyTo` metadata so the Router can route responses back to the correct sender.
- **Use `AsyncLock` for keyed serialization**: When you need "one operation at a time for a given resource" (per-file inbox, per-conversation dispatch, per-path session persist), use `shared/async-lock.ts`. Don't invent ad-hoc `Promise` chains. Same primitive, one mental model. Prior rejections don't deadlock later work; errors don't propagate past their own call. See `messaging/inbox.ts`, `routing/router.ts`, `agents/conversation-manager.ts` for current users.
- **Persist-before-ack**: For any message Rondel has accepted responsibility for, write to disk before the accept returns. In-memory state is a cache; disk is truth. See the Router's `enqueue` (disk write before in-memory push) and the inbox module for the canonical implementations.
...
└── apps/
    ...
    │       │   ├── router.ts         # Router — per-conversation lock, send/queue decisions, recovery
    │       │   └── queue-store.ts    # Disk-backed per-conversation queue (JSON per key, atomic writes)
...
└── state/                       # Runtime ephemera (NOT committed)
    ...
    ├── queues/                  # Per-conversation Router queues (disk = truth)
    │   └── {encoded-key}.json   # Accepted-but-not-yet-drained messages; recovered on startup
