# FlowClaw — Architecture Analysis
...
- **MCP for tool injection is the right call.** It's structured, typed, extensible, and standard. Compared to `--append-system-prompt` (which shoves tool descriptions into text), MCP gives Claude proper tool schemas. Adding new FlowClaw tools later means adding a handler, not rewriting prompt text.
...
- **Inbox polling at 5s intervals.** This is fine for MVP, but inter-agent messages will have 0-5s latency. If agents need tighter coordination, consider `fs.watch` or a lightweight event emitter within the Node process (since the MCP server already runs in the same FlowClaw core process and can signal the inbox poller directly).

## Future-proofness: Strong

This is where FlowClaw's design really shines when compared against OpenClaw's architecture:

1. **Channel adapters are a clean extension point.** The `ChannelAdapter` interface is minimal enough that adding Slack, WhatsApp, or a Web UI requires zero changes to the core. OpenClaw has 17+ channel plugins; FlowClaw's interface is a simplified version of the same pattern. The doc correctly identifies that a Web UI is "just another channel adapter" — this is true and means the architecture doesn't need to change.
...
## Where FlowClaw diverges from OpenClaw (and why that's correct)

| Aspect | OpenClaw | FlowClaw | Verdict |
|--------|----------|----------|---------|
...
2. **Session bloat over time.** The 71h refresh interval is a good default, but long-running agents with many turns will accumulate context. Consider documenting how Claude CLI's session management interacts with FlowClaw's session refresh.
...
4. **No graceful drain on shutdown.** The doc covers crash recovery but doesn't detail what happens to in-flight turns when FlowClaw receives SIGTERM. Consider: wait for active turns to complete (with timeout), then kill child processes.
