# Rondel vs OpenClaw: Comprehensive Audit Verdict

> Generated 2026-03-28. Covers every subsystem in the Rondel codebase compared against OpenClaw (the reference architecture).

---

## Top 5 Rondel Advantages (Don't Regress)

**1. Per-conversation process isolation.** Each `(agentName, chatId)` gets its own Claude CLI process and MCP server. A crash or resource leak in one conversation cannot affect others. OpenClaw multiplexes everything in a single Gateway process. This is Rondel's strongest architectural differentiator.
...
**3. Template-based subagent context.** Rondel's `templates/` directory gives subagents purpose-built system prompts from scratch, while OpenClaw inherits-and-strips parent context. Rondel's approach is cleaner, avoids context leakage, and includes `maxTurns` (which OpenClaw lacks).

**4. Clean `sendOrQueue` pattern with typed DI hooks.** The one-writer-per-conversation invariant is enforced at a single clear boundary (the Router). `RondelHooks` uses compile-time typed events with `readonly` payloads and dependency injection -- more testable and type-safe than OpenClaw's global singleton hook registry.
...
| 2 | **No singleton instance guard** -- two Rondel instances on same project corrupt shared state | Config & State | Undetectable state corruption |
...
- **`FRAMEWORK_DISALLOWED_TOOLS` not applied to subagents**: SubagentProcess doesn't strip the built-in `Agent` tool -- subagents bypass Rondel's delegation system
...
- **Orphaned MCP temp files**: If Rondel crashes, `/tmp/rondel-mcp/` files are never cleaned up
...
| A3 | **Singleton instance guard** -- PID lockfile at `~/.rondel/{projectId}/rondel.lock` | `index.ts` | S |
| A4 | **Error boundary on `RondelHooks.emit()`** -- wrap each listener in try/catch | `hooks.ts` | S |
...
| A8 | **Spawn depth tracking** -- add `RONDEL_SPAWN_DEPTH` env var, strip spawn tools at depth >= 1 | `mcp-server.ts`, `agent-manager.ts` | M |
| A9 | **Apply `FRAMEWORK_DISALLOWED_TOOLS` to subagents** | `subagent-process.ts` | S |
| A10 | **Schema-based config validation** -- Zod schemas for `RondelConfig` and `AgentConfig` | `config.ts` | M |
...
| B10 | **File-based logging** -- JSONL to `~/.rondel/{projectId}/logs/` | `logger.ts` | M |
...
2. **Under "Error Handling Patterns"**: *Hook listeners must be wrapped in error boundaries at the emit site. Use the safe `emit()` override in `RondelHooks` -- never raw `EventEmitter.emit()` for hooks.*
...
5. **Under "Patterns to Follow"**: *Subagent processes must always apply `FRAMEWORK_DISALLOWED_TOOLS` and spawn-depth restrictions. The MCP server must check `RONDEL_SPAWN_DEPTH` and strip delegation tools at the configured max depth.*
...
**Rondel**: Spawns external `claude` CLI child processes via `child_process.spawn()`. Two distinct types: `AgentProcess` (persistent, per-conversation) and `SubagentProcess` (ephemeral, single task). Per-conversation isolation is a stated correctness invariant.

**Verdict**: Intentional architectural difference. Rondel's approach trades startup cost and resource overhead for stronger isolation. No action needed -- document the expected process count ceiling.
...
**Rondel**: Explicit `AgentState` union: `starting -> idle <-> busy`, with `crashed`, `halted`, `stopped`. SubagentProcess: `running -> completed | failed | killed | timeout`.
...
**Rondel advantage**: Explicit typed state machine is more transparent and testable than OpenClaw's implicit lifecycle tracking.

#### Crash Recovery

| Feature | OpenClaw | Rondel |
|---------|----------|----------|
...
**Rondel**: Two-layer persistence (session index + transcripts). Clean separation of session identity from session state. `sessionEstablished` event confirms/updates session IDs. Resume failure detection with fresh-session fallback.
...
**Rondel**: No heartbeat or health monitoring. Only health signal is the state machine.
...
**Rondel**: No orphan recovery. Running subagents are killed on restart, results never delivered, queued messages lost.
...
**Rondel**: Per-conversation state machine + simple FIFO queue. `sendOrQueue` pattern. No global concurrency cap, no debouncing, no coalescing.

**Rondel advantage**: Dramatically simpler while preserving the critical invariant (one writer per conversation).
...
**Rondel**: `SIGINT`/`SIGTERM` handlers call `shutdown()` which stops adapter, scheduler, bridge, kills all processes, persists session index, exits.
...
**Rondel**: Clean minimal `ChannelAdapter` interface with one implementation (`TelegramAdapter`). Multi-account via `Map<string, TelegramAccount>`.
...
**Rondel**: Minimal `ChannelMessage` with 6 fields: `accountId`, `chatId`, `senderId`, `senderName`, `text`, `messageId`.
...
**Rondel**: Simple 1:1 account-to-agent mapping. Bot token = routing.

**Rondel advantage**: 1:1 agent-to-bot model provides natural isolation and simpler reasoning.

**Recommendations**: Add optional `bindings` config for group routing (Important). Keep 1:1 as primary mode (Nice-to-have).

#### Outbound Delivery

| Aspect | OpenClaw | Rondel |
|--------|----------|----------|
...
**Rondel advantage**: Markdown-with-fallback approach is pragmatic and cleaner than per-account config.
...
**Rondel**: Single global `allowedUsers` set. Silent drop for unauthorized users.
...
**Rondel advantage**: `sendOrQueue` pattern is cleaner and more explicit than OpenClaw's multi-phase dispatch.
...
**Rondel advantage**: Text-based crash/halt notifications are more informative than emoji reactions.
...
**Rondel**: Non-blocking spawn via MCP tool + Bridge. Hook-based delivery via `sendOrQueue`. Simple text block delivery with instruction to parent to summarize.
...
**Rondel advantage**: `sendOrQueue` is simpler and more debuggable than OpenClaw's multi-phase dispatch.
...
**Rondel**: In-memory `Map` with 10-field records, 1-hour TTL pruning.
...
**Rondel advantages**: Template-based context is cleaner. `maxTurns` parameter gives explicit loop control OpenClaw lacks.
...
**Gap**: Subagents get full Rondel MCP server including spawn tools. No depth-based restriction.
...
**Rondel**: One kind (`every`) with human-readable durations. Two session targets. One payload type. Two delivery modes (none, announce).
...
**Rondel**: Single backoff strategy (identical delays to OpenClaw), no error classification, no auto-disable.

**Rondel advantage**: Simpler backoff implementation with identical delays in a fraction of the code.
...
**Rondel advantages**: Config-driven jobs in `agent.json` (simpler), file-watch hot-reload (more natural), projectId-scoped state.
...
**Rondel advantages**: Sequential execution avoids concurrency bugs. Clear missed-job stagger. No cumulative catch-up.
...
**Rondel**: Separate MCP server process with HTTP bridge, per-conversation isolation, env-var context.

**Rondel advantage**: Per-conversation process isolation is genuinely better for security. Keep the MCP sidecar architecture.
...
**Rondel**: 2-layer filtering. Subagents have no tool restrictions beyond template config.
...
Rondel's 8 tools + Claude CLI built-ins provide good coverage. The effective gap is smaller than it appears.
...
**Rondel**: Plain JSON with manual imperative validation, env substitution, scheduler-only hot-reload.
...
**Rondel**: Env-var-only secrets. No file locking. No maintenance. Fire-and-forget session persistence.
...
**Rondel advantage**: Simpler state model with flat key scheme is more transparent and debuggable.

#### Logging & Errors

**OpenClaw**: Rolling JSON Lines files, console + file levels, redaction, rich error utilities, typed backoff policy.

**Rondel**: Console-only with color, level filtering, component prefix, `child()` for scoped sub-loggers.

**Rondel advantage**: Interface-based logger with DI via `child()` is cleaner and more testable.
...
**Rondel**: Two-layer context concatenation, direct `readFile`/`writeFile`, no locking.

**Rondel advantage**: Simpler context model is immediately understandable and fully predictable.
...
**Rondel**: `SIGINT`/`SIGTERM` handlers, ordered shutdown sequence.
...
**Rondel**: 5 hooks on a single EventEmitter: `subagent:spawning`, `subagent:completed`, `subagent:failed`, `cron:completed`, `cron:failed`.

**Verdict**: Rondel covers the right things for the current feature set. Don't try to reach 40 hooks.
...
**Rondel**: Fire-and-forget only. No pipeline/reducer pattern.
...
**Rondel**: Hardcoded in `index.ts`. No registration API.

**Recommendation**: Extract to `src/hook-wiring.ts` (Important). Defer discovery system (Nice-to-have).

#### Error Handling in Hooks

**Critical finding**: Rondel's `EventEmitter.emit()` has no error boundary. A throwing listener will crash the emitter (AgentManager or Scheduler), prevent other listeners from running, and potentially crash the entire system. This violates the stated CLAUDE.md invariant.

**Recommendation**: Override `emit()` in `RondelHooks` to wrap each listener in try/catch (Critical).
...
**Rondel**: Single `sessions.json` with 5 fields, simple `agentName:chatId` keys.

**Rondel advantage**: Simple key model is easy to reason about, debug, and operate manually.
...
**Rondel**: Manual `/new` only. No automatic reset. Sessions live forever.

**Recommendations**: Add idle-based reset (Important), add daily reset option (Important).

#### Resume Failure Handling

**Rondel advantage**: `RESUME_FAILURE_WINDOW_MS` heuristic is pragmatic and well-adapted to CLI-based architecture. OpenClaw doesn't need this (different architecture) but Rondel handles it cleanly.
...
**Rondel**: Flat JSONL with session header, user entries, and raw stream-json events.

**Rondel advantage**: Raw event capture preserves full Claude CLI protocol fidelity.
...
**Rondel**: No maintenance whatsoever. Sessions and transcripts grow unbounded.
...
**Rondel**: Two-layer concatenation at spawn time. Claude CLI owns context management.
...
**Rondel**: Only `total_cost_usd` from result events, not persisted.
...
**Rondel**: Transcript-only persistence. No memory tools. Agents lose all knowledge on reset/compaction.
...
**OpenClaw**: ~90+ fields. **Rondel**: 5 fields.
