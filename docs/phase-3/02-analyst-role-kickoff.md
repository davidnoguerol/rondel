...
Rondel is a multi-agent orchestration framework built on the Claude CLI. The long-term vision is an **agentic self-evolving harness** that manages real operations. Phase 1 plans the orchestrator (the coordinator who never does specialist work) — kickoff only ([`docs/phase-1/04-orchestrator-role-kickoff.md`](../phase-1/04-orchestrator-role-kickoff.md)), not yet designed or built. Heartbeats and the task board are built; treat role infrastructure as a pending dependency, not an existing API. Phase 3 adds the **analyst** (the agent that watches the system and helps it improve).
...
### Dependencies
Phase 1 items 1, 2, 3, 4 (heartbeat, tasks, goals, orchestrator). Phase 2 items 1, 3 (memory, guardrails). Phase 3 item 1 (experiments). The theta wave is the system-level consumer of all of these. Built today: heartbeat, tasks, memory (plus the knowledge base, relevant for reading agents' learnings). Pending: goals, orchestrator, guardrails — verify before starting this design.
...
**Focus**: map the analyst + theta-wave. Key files: `templates/analyst/` (the whole directory — IDENTITY, SOUL, GOALS, CLAUDE.md, config.json), `templates/analyst/.claude/skills/theta-wave/SKILL.md` (8-phase deep cycle), `templates/analyst/.claude/skills/system-diagnostics/SKILL.md`, `templates/analyst/experiments/` (the analyst's own autoresearch cycle on system-effectiveness), cron setup for nightly theta wave, the orchestrator ↔ analyst conversation protocol (how they debate via the inbox). Cover: analyst-only skills (have the subagent diff against another template's skills to identify the analyst-only subset), analyst-only tools (if any), what metrics the analyst collects, how the system-effectiveness score gets computed, how structural proposals flow from analyst to orchestrator (to user).
...
1. **Role infrastructure** — from Phase 1 item 4 (assumed built). Adding analyst = new value for the `role` field + new template + new skill pack + new tool allowlist. Status check before designing: at the time of writing there is no `role` field in `AgentConfig` — if Phase 1 item 4 still isn't built, this design must specify the role enum itself or be explicitly blocked on it.
...
5. **Analyst-only tools** — `rondel_experiment_cycle_configure` (from item 1), `rondel_system_effectiveness_score`, `rondel_analyst_report_to_user`. `rondel_heartbeat_read_all` already exists (admin-only, in `bridge/mcp-server.ts`) — reuse it; design only the genuinely new analyst tools. Schemas + privilege.
...
12. **Ledger events** — `theta_wave_started`, `theta_wave_completed`, `analyst_proposal`, `analyst_report_sent` (ledger kinds are snake_case — see `ledger/ledger-types.ts`; colon-separated names are the hook-event convention).
