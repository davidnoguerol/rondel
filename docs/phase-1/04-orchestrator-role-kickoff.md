...
**Focus**: does OpenClaw distinguish agent roles? Coordinator vs worker vs observer patterns? Is there an agent-vs-subagent hierarchy that maps to this? Any equivalent of an `admin` flag or role gating for privileged tools? How are privileged operations (managing other agents, setting config, spawning work) gated? OpenClaw has an `ownerOnly` flag on gateway/cron tools — explore that pattern directly in its source.
...
9. **Orchestrator-only MCP tools list** — items 1–2 have shipped, so start from what exists: (a) `rondel_heartbeat_read_all` is **built**, registered admin-only inside the `IS_ADMIN` block of `apps/daemon/src/bridge/mcp-server.ts`; `01-heartbeat-design.md` §4 already specs widening the gate to `isAdmin || role === "orchestrator"` as a one-line change. (b) The task board shipped **without** a dispatch tool — dispatch today is `rondel_task_create` + `rondel_send_message`; whether to add a batched `rondel_task_dispatch_many` (pure sugar over that pair, per `02-task-board-design.md` §5/§16) is this design's call. (c) `rondel_goals_*` tools (item 3) are **not built** — spec which of them are orchestrator-gated. New tool `rondel_find_orchestrator` for cross-agent discovery.
