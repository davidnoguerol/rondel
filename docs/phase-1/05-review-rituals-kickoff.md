...
**Important dependency**: this item sits on top of items 1–4 (heartbeat, task board, goals, orchestrator role). Items 1–2 are **built** — design against their real shipped tool surfaces (`rondel_heartbeat_update` / `rondel_heartbeat_read_all` and the nine `rondel_task_*` tools; see ARCHITECTURE.md §6b and the task tool table, and read the tool descriptions in `apps/daemon/src/bridge/mcp-server.ts`). Items 3–4 are not built yet — if you need to refer to a proposed tool from them (e.g., `rondel_goals_set_daily_focus` from item 3), do so and note the dependency.
...
3. **Skill prose content** — concrete steps for each ritual. Base on CortexOS's proven flow (per subagent report) but adapted to Rondel's tool surface (use `rondel_heartbeat_read_all`, `rondel_task_list`, `rondel_goals_set_daily_focus`, `rondel_ledger_query`, `rondel_send_message`, etc.). Note: `rondel_heartbeat_read_all` exists but is admin-gated today (registered inside the `IS_ADMIN` block in `bridge/mcp-server.ts`); the ritual skill depends on item 4 widening the gate to `role: orchestrator` — the one-line change planned in `01-heartbeat-design.md` — or on the orchestrator being admin. Write the actual skill Markdown that will ship.
...
- **Items 1–2 are built; assume items 3–4 will be.** Honor the shipped heartbeat/task tool contracts as-is (`rondel_heartbeat_read_all`, `rondel_task_create`, etc.) — e.g. `rondel_task_complete` requires a `result` string, `rondel_task_list` supports `staleOnly`, and denial of an `externalAction` completion flips the task to blocked. Refer to items 3–4's proposed tools (`rondel_goals_*`, role gating) freely even though they don't exist yet, and flag any assumption about their surface that might not hold.
